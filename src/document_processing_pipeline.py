...
def validate_passport_in_certificate(ocr_text: str) -> bool:
    ...
    logger.debug(f"🔍 Passport validation: Found {indicator_count} keywords + {pattern_matches} patterns = {total_indicators} total indicators")
...
def validate_emirates_id_in_certificate(ocr_text: str) -> bool:
    ...
    logger.debug(f"🔍 Emirates ID validation: Found {indicator_count} keywords + {pattern_match} patterns = {total_indicators} total indicators")
...
def validate_certificate_in_emirates_id(ocr_text: str) -> bool:
    ...
    logger.debug(f"🔍 Certificate validation: Found {indicator_count} keywords + {pattern_matches} patterns = {total_indicators} total indicators")
...
def validate_attestation_in_certificate(ocr_text: str) -> bool:
    ...
    logger.debug(f"🔍 Attestation validation: Found {indicator_count} keywords + {pattern_matches} patterns = {total_indicators} total indicators")
...
def validate_attestation_label_detection(yolo_labels: list, ocr_text: str) -> bool:
    ...
    if yolo_detected_attestation:
        logger.debug(f"🔍 YOLO detected attestation labels: {yolo_labels}")
        # Additional validation with OCR text
        if validate_attestation_in_certificate(ocr_text):
            logger.info("✅ YOLO attestation detection confirmed by OCR text")
            return True
        else:
            logger.warning("⚠️ YOLO detected attestation but OCR text doesn't confirm")
            return False
...
def validate_document_misclassification(resnet_label: str, ocr_text: str, yolo_labels: list = None) -> str:
    ...
    logger.debug(f"🔍 Running misclassification validation for '{resnet_label}'...")
    ...
    if yolo_labels and validate_attestation_label_detection(yolo_labels, ocr_text):
        logger.info("🎯 YOLO detected attestation_label - switching to attestation_label")
    ...
    if resnet_label == "certificate":
        if validate_passport_in_certificate(ocr_text):
            logger.warning("⚠️ Strong passport indicators found in 'certificate' - switching to passport_1")
            return "passport_1"
        elif validate_emirates_id_in_certificate(ocr_text):
            logger.warning("⚠️ Strong Emirates ID indicators found in 'certificate' - switching to emirates_id")
            return "emirates_id"
        elif validate_attestation_in_certificate(ocr_text):
            logger.warning("⚠️ Strong attestation indicators found in 'certificate' - switching to certificate_attestation")
            return "certificate_attestation"
        else:
            logger.info("✅ Certificate classification confirmed - no conflicting indicators found")
    ...
    # Check for certificate misclassified as emirates_id
        ...
        if validate_certificate_in_emirates_id(ocr_text):
            logger.warning("⚠️ Strong certificate indicators found in 'emirates_id' - switching to certificate")
            return "certificate"
        elif validate_attestation_in_certificate(ocr_text):
            logger.warning("⚠️ Strong attestation indicators found in 'emirates_id' - switching to certificate_attestation")
            return "certificate_attestation"
        else:
            logger.info("✅ Emirates ID classification confirmed - no conflicting indicators found")
    
    # Check for passport misclassified as unknown
    elif resnet_label == "unknown":
        if validate_passport_in_certificate(ocr_text):
            logger.warning("⚠️ Passport indicators found in 'unknown' - switching to passport_1")
            return "passport_1"
        elif validate_emirates_id_in_certificate(ocr_text):
            logger.warning("⚠️ Emirates ID indicators found in 'unknown' - switching to emirates_id")
            return "emirates_id"
        elif validate_attestation_in_certificate(ocr_text):
            logger.warning("⚠️ Attestation indicators found in 'unknown' - switching to certificate_attestation")
            return "certificate_attestation"
...
def classify_and_ocr(image_path: str, temp_dir: str):
    ...
    logger.info(f"✂️ Cropped to: {cropped_path}")
    ...
    logger.info(f"🏷️ ResNet label: {resnet_label}")
    ...
    if final_label in ["unknown", "certificate"]:
        logger.debug(f"🔍 Using Gemini Vision fallback for {final_label} classification...")
        gemini_vision_label = classify_image_with_gemini_vision(cropped_path)
        if gemini_vision_label != "unknown":
            logger.info(f"🤖 Gemini Vision classified as: {gemini_vision_label}")
            ...
            logger.info(f"🤖 Gemini text fallback label: {gemini_text_label}")
    ...
    if final_label in ["passport_photo", "personal_photo", "certificate", "certificate_attestation", "attestation_label"]:
        logger.debug(f"🔍 Checking orientation for {final_label}...")
        orientation_analysis = check_image_orientation(cropped_path)
        
        if not orientation_analysis.get("orientation_correct", True):
            logger.warning(f"⚠️ Orientation issues detected: {orientation_analysis.get('issues', [])}")
            logger.info(f"💡 Recommendations: {orientation_analysis.get('recommendations', [])}")
            ...
            if corrected_path != cropped_path:
                ...
                logger.info(f"✅ Image auto-corrected and saved as: {corrected_path}")
